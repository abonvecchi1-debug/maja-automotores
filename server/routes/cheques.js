...
const map = (r) => ({
  ...
  purchaseTransactionId: r.purchase_transaction_id ?? undefined,
  createdAt: r.created_at,
});

router.get('/', (req, res) => {
  const { estado, tipo, from, to, recibido_de, librador, moneda } = req.query;
  ...
  if (recibido_de) { q += ' AND recibido_de = ?';         params.push(recibido_de); }
  if (librador)    { q += ' AND librador = ?';            params.push(librador); }
  if (moneda)      { q += ' AND moneda = ?';              params.push(moneda); }
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE cheques SET
    ...
    estado=?,observaciones=?,purchase_vehicle_id=?,purchase_asset_id=?,purchase_transaction_id=?
    WHERE id=?`)
    .run(
      b.numero ?? ex.numero,
      ...
      b.purchaseTransactionId !== undefined ? b.purchaseTransactionId : (ex.purchase_transaction_id ?? null),
      id,
    );
